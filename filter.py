#filter only for the sentences that match our criteria

import pandas as pd

# Load the CSV files
csv1 = pd.read_csv('updated_ner_data.csv')
...
grouped = filtered_csv1.groupby(['PMID', 'Sentence id']).filter(
...
merged_df = pd.merge(grouped, csv2, left_on=['PMID', 'Sentence id'], right_on=['PMID', 'Sentence id'])
